...
void DrawPlayerEsp()
{
    ...
    for (auto entity : EngineInstance->GetActors())
    {
        ...
        // Set specific colors and font sizes for different roles
        ...
        if (entity->GetActorRole() == EActorRole::EActorRole_Window) {
            config.TextColour = Colour(255, 255, 0); // Example: setting color to yellow window
            config.FontSize = 30; // Set a larger font size for EActorRole_Window
        }
        if (entity->GetActorRole() == EActorRole::EActorRole_Players) {
            config.FontSize = 35; // Set a larger font size for EActorRole_Players
        }
        if (entity->GetActorRole() == EActorRole::EActorRole_Pallet) {
            config.TextColour = Colour(255, 181, 208); // Example: setting color to pink pallet
        }
        if (entity->GetActorRole() == EActorRole::EActorRole_MeatHook) {
            config.TextColour = Colour(255, 165, 80); // Example: setting color to orange meat hook
        }
        ...
        if (entity->GetActorRole() == EActorRole::EActorRole_Players) {
            // Only draw distance for EActorRole_Players
            std::wstring wdistance = (config.Distance) ? L"[" + std::to_wstring((int)distance / 2) + L"m]" : L"";
            if (distance > config.MaxDistance)
                continue;

            DrawText(screenpos.x, screenpos.y, wdistance, "Verdana", config.FontSize, config.TextColour, CentreCentre);
        }
        else {
            // Draw name and distance for other roles
