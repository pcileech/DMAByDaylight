...
void DrawGeneratorEsp()
{
    ...
    for (auto entity : EngineInstance->GetActors())
    {
        ...
        if (percent <= 39)
            config.TextColour = Colour(255, 255, 255); // White
        else if (percent <= 69)
            config.TextColour = Colour(255, 105, 180); // Bright Pink
        else
            config.TextColour = Colour(255, 0, 0); // Bright Red

        // Increase font size for percentage
        float percentageFontSize = config.GenESPFontSize * 1.5f; // Increase font size by 50%
